...
The registry gate also binds packet repository/SHA/environment/owner/reviewer/decision times to the public snapshot. A public snapshot alone is insufficient. Historical samples without packets remain archived. The 2026-09-06 registry points to new owner-reviewed scoped snapshots and verified packets in `test-records/releases/2026-09-06/`, with separately stored authority records under `data/authority/`. Self-review is disclosed, not represented as independent review. Approved expiry is fixed; this does not introduce automatic source polling or automatic renewal.
